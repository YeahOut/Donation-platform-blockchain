...
import { PrismaClient, DonationStatus } from '../generated/prisma';
...
app.post('/api/login', async (req: Request, res: Response) => {
  const schema = z.object({ username: z.string(), password: z.string().min(4) });
...
// 내부 유틸: 토큰 이동 트랜잭션
async function moveBankToDonation(amount: number) {
  const bank = await prisma.token.findUnique({ where: { type: 'BANK' } });
  const donation = await prisma.token.findUnique({ where: { type: 'DONATION' } });
  if (!bank || !donation) throw new Error('Tokens not initialized');
  if (bank.balance < amount) throw new Error('Bank token insufficient');
...
app.post('/api/donate', authMiddleware, async (req: AuthRequest, res: Response) => {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      const donation = await tx.donation.create({ data: { userId, amount, status: DonationStatus.PENDING } });
...
// 5. 기부 성공/실패 api
app.post('/api/donation/:id/settle', authMiddleware, async (req: AuthRequest, res: Response) => {
  const schema = z.object({ status: z.enum(['SUCCESS', 'FAILED']) });
  const parsed = schema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ message: 'Invalid input' });
...
app.post('/api/dev/seed', async (_req: Request, res: Response) => {
  ...
  // Initialize tokens
  await prisma.token.upsert({ where: { type: 'BANK' }, update: { balance: 1_000_000 }, create: { type: 'BANK', balance: 1_000_000 } });
  ...
  async function ensureBalance(userId: number, target: number, note: string) {
  ...
  await ensureBalance(tester.id, 100_000, 'adjust to target (tester)');
  await ensureBalance(molly.id, 15_000, 'adjust to target (molly)');
  await ensureBalance(sol.id, 1_200, 'adjust to target (sol)');
